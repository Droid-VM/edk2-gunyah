  ...
  )
{
  ...
  //
  // 7.c. Configure the hiprio queue (queue 0). We don't use it, but crosvm
  // requires all queues to be activated before DRIVER_OK.
  //
  Status = VirtioFs->Virtio->SetQueueSel (
                               VirtioFs->Virtio,
                               VIRTIO_FS_HIPRIO_QUEUE
                               );
  if (EFI_ERROR (Status)) {
    goto Failed;
  }

  {
    ...
    Status = VirtioRingMap (
               ...
               &VirtioFs->HipriRing,
               &RingBaseShift,
               &VirtioFs->HipriRingMap
               );
    if (EFI_ERROR (Status)) {
      goto ReleaseHipriQueue;
    }
    ...
    if (EFI_ERROR (Status)) {
      goto UnmapHipriQueue;
    }
  }

  //
  // 7.d. Fetch and remember the number of descriptors we can place on the
  // request queue at once. We'll need two descriptors per request, as a
  // minimum -- request header, response header.
  //
  Status = VirtioFs->Virtio->SetQueueSel (
                               VirtioFs->Virtio,
...
UnmapHipriQueue:
  VirtioFs->Virtio->UnmapSharedBuffer (VirtioFs->Virtio, VirtioFs->HipriRingMap);

ReleaseHipriQueue:
  VirtioRingUninit (VirtioFs->Virtio, &VirtioFs->HipriRing);
