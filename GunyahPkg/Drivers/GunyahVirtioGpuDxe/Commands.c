  ...
  )
{
  ...
  // the device. Initialize all queues (control + cursor).
  //
  for (QueueIdx = 0; QueueIdx < VIRTIO_GPU_NUM_QUEUES; QueueIdx++) {
    ...
    //
    Status = VirtioRingInit (VgpuDev->VirtIo, QueueSize, &VgpuDev->Ring[QueueIdx]);
    if (EFI_ERROR (Status)) {
      goto RollbackQueues;
    }
    ...
    Status = VirtioRingMap (
               VgpuDev->VirtIo,
               &VgpuDev->Ring[QueueIdx],
               &RingBaseShift,
               &VgpuDev->RingMap[QueueIdx]
               );
    if (EFI_ERROR (Status)) {
      VirtioRingUninit (VgpuDev->VirtIo, &VgpuDev->Ring[QueueIdx]);
      goto RollbackQueues;
    }
    ...
    if (EFI_ERROR (Status)) {
      VgpuDev->VirtIo->UnmapSharedBuffer (VgpuDev->VirtIo, VgpuDev->RingMap[QueueIdx]);
      VirtioRingUninit (VgpuDev->VirtIo, &VgpuDev->Ring[QueueIdx]);
      goto RollbackQueues;
    }
  }
  ...
  Status       = VgpuDev->VirtIo->SetDeviceStatus (VgpuDev->VirtIo, NextDevStat);
  if (EFI_ERROR (Status)) {
    QueueIdx = VIRTIO_GPU_NUM_QUEUES;
    goto RollbackQueues;
  }
  ...
  )
{
  ...
  //
  VirtioPrepare (&VgpuDev->Ring[VIRTIO_GPU_CONTROL_QUEUE], &Indices);
  VirtioAppendDesc (
    &VgpuDev->Ring[VIRTIO_GPU_CONTROL_QUEUE],
    RequestDeviceAddress,
    (UINT32)RequestSize,
    VRING_DESC_F_NEXT,
    &Indices
