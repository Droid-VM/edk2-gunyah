  ...
  )
{
  ...
  //
  // step 4b -- allocate and configure all three queues:
  //   queue 0 = controlq, queue 1 = eventq, queue 2 = requestq
  // crosvm requires all three to be ready before DRIVER_OK.
  //

  //
  // --- controlq (queue 0) ---
  //
  Status = Dev->VirtIo->SetQueueSel (Dev->VirtIo, VIRTIO_SCSI_CONTROL_QUEUE);
  if (EFI_ERROR (Status)) {
    goto Failed;
  }
  ...
  Status = VirtioRingInit (Dev->VirtIo, QueueSize, &Dev->ControlRing);
  if (EFI_ERROR (Status)) {
    goto Failed;
  }
  ...
  if (EFI_ERROR (Status)) {
    goto ReleaseControlQueue;
  }

  Status = Dev->VirtIo->SetQueueNum (Dev->VirtIo, QueueSize);
  if (EFI_ERROR (Status)) {
    goto UnmapControlQueue;
  }
  ...
  // --- eventq (queue 1) ---
  //
  Status = Dev->VirtIo->SetQueueSel (Dev->VirtIo, VIRTIO_SCSI_EVENT_QUEUE);
  if (EFI_ERROR (Status)) {
    goto UnmapControlQueue;
  }
  ...
  Status = VirtioRingInit (Dev->VirtIo, QueueSize, &Dev->EventRing);
  if (EFI_ERROR (Status)) {
    goto UnmapControlQueue;
  }
  ...
  if (EFI_ERROR (Status)) {
    goto ReleaseEventQueue;
  }

  Status = Dev->VirtIo->SetQueueNum (Dev->VirtIo, QueueSize);
  if (EFI_ERROR (Status)) {
    goto UnmapEventQueue;
  }
...
UnmapEventQueue:
  Dev->VirtIo->UnmapSharedBuffer (Dev->VirtIo, Dev->EventRingMap);

ReleaseEventQueue:
  VirtioRingUninit (Dev->VirtIo, &Dev->EventRing);

UnmapControlQueue:
  Dev->VirtIo->UnmapSharedBuffer (Dev->VirtIo, Dev->ControlRingMap);

ReleaseControlQueue:
  VirtioRingUninit (Dev->VirtIo, &Dev->ControlRing);

Failed:
  //
  // Perform a full device reset so crosvm clears internal queue activation
  // state. This prevents "queue is already activated" on retry.
  //
