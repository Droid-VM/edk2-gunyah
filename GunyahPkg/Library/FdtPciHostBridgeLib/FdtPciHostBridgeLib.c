/** @file
  PCI Host Bridge Library instance for pci-cam-generic DT nodes
...
//
// We expect the "ranges" property of "pci-host-cam-generic" to consist of
// records like this.
//
#pragma pack (1)
typedef struct {
...
#define DTB_PCI_HOST_RANGE_TYPEMASK      (BIT25 | BIT24)
  ...
  )
{
  ...
  *Mmio64Size   = 0;
  ...
  //
  // Clamp MMIO ranges to the CPU's physical address space.
  //
  AddressLimit = LShiftU64 (1ULL, ArmGetPhysicalAddressBits ());
  ...
  *BusMax = SwapBytes32 (((CONST UINT32 *)Prop)[1]);
  ...
  //
  // Sanity check: the config space must accommodate all 256 register bytes of
  // all 8 functions of all 32 devices of all buses (CAM: 256 bytes per function).
  //
  if ((*BusMax < *BusMin) || (*BusMax - *BusMin == MAX_UINT32) ||
      (DivU64x32 (ConfigSize, 256 * 8 * 32) < *BusMax - *BusMin + 1))
  {
    DEBUG ((
      DEBUG_ERROR,
       ...
       ++RecordIdx)
  {
    ...
    switch (SwapBytes32 (Record->Type) & DTB_PCI_HOST_RANGE_TYPEMASK) {
      ...
      case DTB_PCI_HOST_RANGE_MMIO64:
      {
        UINT64  RangeBase;
        UINT64  RangeSize;
        UINT64  RangeTranslation;

        RangeBase        = SwapBytes64 (Record->ChildBase);
        RangeSize        = SwapBytes64 (Record->Size);
        RangeTranslation = SwapBytes64 (Record->CpuBase) - RangeBase;

        if (RangeTranslation != 0) {
          DEBUG ((
            DEBUG_ERROR,
        ...
        //
        // If this 64-bit typed range fits entirely within 4GB and we have
        // no MMIO32 range yet, use it as the MMIO32 range. Some DTs mark
        // sub-4GB ranges with the 64-bit space type.
        //
        if ((*Mmio32Size == 0) &&
            (RangeBase <= MAX_UINT32) && (RangeSize <= MAX_UINT32) &&
            (RangeBase + RangeSize <= SIZE_4GB))
        {
          *Mmio32Base       = RangeBase;
          *Mmio32Size       = RangeSize;
          Mmio32Translation = RangeTranslation;

          ASSERT (PcdGet64 (PcdPciMmio32Translation) == Mmio32Translation);

          DEBUG ((
            DEBUG_INFO,
            "%a: promoting MMIO64 range [0x%Lx+0x%Lx) to MMIO32\n",
            __func__,
            RangeBase,
            RangeSize
            ));
        } else if (RangeBase < SIZE_4GB) {
          //
          // The range starts below 4GB but extends above it. Trim the
          // below-4GB portion so MemAbove4G.Base >= 4GB as required.
          //
          UINT64  Trim;

          Trim = SIZE_4GB - RangeBase;
          if (Trim < RangeSize) {
            *Mmio64Base       = SIZE_4GB;
            *Mmio64Size       = RangeSize - Trim;
            Mmio64Translation = RangeTranslation;

            //
            // Clamp to CPU physical address limit.
            //
            if (*Mmio64Base + *Mmio64Size > AddressLimit) {
              *Mmio64Size = AddressLimit - *Mmio64Base;
            }
            ...
            DEBUG ((
              DEBUG_INFO,
              "%a: trimmed MMIO64 range from [0x%Lx+0x%Lx) to [0x%Lx+0x%Lx)\n",
              __func__,
              RangeBase,
              RangeSize,
              *Mmio64Base,
            ...
            DEBUG ((
              DEBUG_WARN,
              "%a: MMIO64 range [0x%Lx+0x%Lx) entirely below 4GB, skipping\n",
              __func__,
              RangeBase,
              RangeSize
              ));
          ...
          *Mmio64Base       = RangeBase;
          *Mmio64Size       = RangeSize;
          Mmio64Translation = RangeTranslation;
      ...
      default:
        DEBUG ((
          DEBUG_WARN,
          "%a: ignoring unrecognized range type 0x%x\n",
          __func__,
          SwapBytes32 (Record->Type)
          ));
        break;
    }
  }
  ...
  )
{
  ...
  return PciHostBridgeUtilityGetRootBridges (
           ...
           TRUE,
           TRUE,     // NoExtendedConfigSpace: CAM only has 256 bytes per function
           BusMin,
           BusMax,
           &Io,
           &Mem,
