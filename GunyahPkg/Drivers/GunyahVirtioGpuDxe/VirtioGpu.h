...
//
// Queue number for sending cursor commands.
//
#define VIRTIO_GPU_CURSOR_QUEUE  1

//
// Total number of virtqueues for the GPU device.
//
...
typedef struct {
  ...
  //
  // VirtIo rings used for VirtIo communication (control + cursor queues).
  //
  VRING                       Ring[VIRTIO_GPU_NUM_QUEUES];

  //
  // Tokens associated with Ring mappings for bus master common buffer
  // operation, from VirtioRingMap().
  //
  VOID                        *RingMap[VIRTIO_GPU_NUM_QUEUES];
