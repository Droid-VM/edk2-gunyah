  ...
  )
{
  ...
  while (Left > 0) {
    ...
    //
    // Cap each FUSE_READ to MaxWrite, which the host negotiated during
    // FUSE_INIT. The host may not be able to allocate a larger buffer.
    //
    ReadSize = (UINT32)MIN ((UINTN)VirtioFs->MaxWrite, Left);
    Status   = VirtioFsFuseReadFileOrDir (
                 VirtioFs,
                 VirtioFsFile->NodeId,
                 VirtioFsFile->FuseHandle,
