/** @file
  ...
  Gunyah variant: Allocates shared pages from restricted-dma-pool when available.
  ...
  )
{
  ...
  //
  // If a restricted DMA pool is available (Gunyah protected VM), allocate
  // from it so that the host/VMM can access the shared buffers.
  //
  if (GunyahDmaPoolIsAvailable ()) {
    Status = GunyahDmaPoolAllocatePages (NumPages, HostAddress);
    if (!EFI_ERROR (Status)) {
      return EFI_SUCCESS;
    }
    ...
    DEBUG ((
      DEBUG_ERROR,
      "%a: DMA pool allocation failed (%r), cannot fall back\n",
      __func__,
      Status
      ));
    return Status;
  ...
  //
  // No DMA pool (non-protected VM) — use standard UEFI allocation.
  //
  *HostAddress = AllocatePages (NumPages);
  if (*HostAddress == NULL) {
...
{
  if (GunyahDmaPoolIsAvailable ()) {
    GunyahDmaPoolFreePages (NumPages, HostAddress);
    return;
  }
...
{
  //
  // Identity mapping — the DMA pool region is at a known physical address
  // that is directly accessible by both guest and host.
  //
